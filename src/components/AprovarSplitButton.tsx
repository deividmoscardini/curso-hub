...
export function AprovarSplitButton({ perfilId, nome, onAprovado }: Props) {
  ...
  const mut = useMutation({
    ...
    onSuccess: (_, escolha) => {
      ...
      toast.success(`${nome} aprovado como ${label}`);
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label="Escolher papel"
          >
            <ChevronDown className="h-4 w-4" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-64">
          <div className="px-2 py-1.5 text-[10px] uppercase text-muted-foreground">Aprovar como…</div>
          ...
          <div className="px-2 py-1 text-[10px] text-muted-foreground">
            Todo aprovado tem acesso a todos os produtos automaticamente.
          </div>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
